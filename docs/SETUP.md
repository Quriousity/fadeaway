...
> 이미 이전 버전의 `schema.sql` 을 적용한 DB라면 아래 패치도 실행할 것.
> 새로 구축하는 경우엔 `schema.sql` 에 이미 반영돼 있다.
> - `supabase/0008_id_editable.sql` — 아이디 변경 허용 (설계문서 D-012)
> - `supabase/0009_realtime_rls.sql` — Realtime 인가 (**앱 배포와 짝**, 아래 주의 참조)

> ⚠️ **0009 는 앱 코드와 짝이다.** 클라이언트가 채널을 `private: true` 로 열어야 통한다.
> SQL 만 적용하고 앱이 옛 버전이면 실시간 메시지가 끊긴다
> (새로고침하면 히스토리는 보인다 — 저장은 별개 경로라서). 둘을 같이 배포할 것.
